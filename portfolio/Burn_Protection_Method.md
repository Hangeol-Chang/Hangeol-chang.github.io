...
최근 3년(2020년도 기준)동안 일어난 화상사고의 26.9%가 4세 이하의 영유아에게서 발생하였고, 이중 65.9%가 집안에서 발생하였다. 따라서, 본 연구는 팔찌형 장치를 이용하여 아이가 열원에 접근하는지를 판단하며, 아이가 열원에 접근하여 화상을 당할 수 있는 상황이 발생하면, 첫 번째로 사이버-물리 시스템을 이용하여 열원과 먼 곳에서 발생되는 소리로 아이를 열원에서 멀어지게 하고, 두 번째로 보호자의 휴대폰으로 알람을 보내어 2중으로 자녀를 보호하도록 한다.
...
- 성균관대 주관 전국 CONNECT 창의연구 2등상 수상
